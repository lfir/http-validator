...
/**
 * Service to send email notifications using an external SMTP service.
 */
@Service
public class EmailNotificationService {
    ...
    void sendPlainTextEmail(String subject, String body) throws ConnectIOException {
        ...
        try {
            ...
            logger.info("Email delivery request submitted via SMTP.");
        } catch (MailException e) {
            String errmsg = "Request for delivery of the Notification Email could not be completed via SMTP.";
            logger.error(errmsg);
     ...
     * <ul>
     * <li>The "notifications.from" property, which should contain the sender's
     * email address.</li>
     * <li>The "notifications.to" property, which should contain the recipient's
     * email address.</li>
     * <li>The {@code mailSender} object, which must not be null.</li>
     * </ul>
     * If any of these properties are blank or missing, the configuration is
     * considered invalid.
     *
     * @return True if all necessary properties are set and non-blank, false
     *         otherwise.
     */
    public boolean isValidConfig() {
        return !(isBlank(this.getFrom()) || isBlank(this.getTo()) || isNull(this.mailSender));
    }
     ...
     */
    String getFrom() {
        return this.env.getProperty(FROM_PROPERTY);
    }
     ...
     */
    String getTo() {
        return this.env.getProperty(TO_PROPERTY);
    }
    ...
    void setLogger(Logger logger) {
        EmailNotificationService.logger = logger;
    }
