...
The ability to configure tasks and schedules easily, combined with prompt email notifications for issues and low resource usage, makes it a practical tool
...
3. **Specify Attributes for Each Task**: For each `validation` element, you need to specify several attributes:
   ...
   separated by tabs (`&#9;`) if there are multiple headers (up to 4000 characters). Can be left empty for no headers.
...
1. **Example Task 1**:
  ...
  - **Request Headers**: Two headers: `Authorization` with a bearer token and `Accept` with value `application/json`, separated by `&#9;` (tab character)
...
- Apart from the API, the run schedule can also be set through a separate configuration file.
Restarting the application manually after a change is also required in this case.
